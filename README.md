...
## 审核时间
小程序的审核在 1 小时到 N 天不等。官方说法是不超过 7 个工作日，一般在 3 天内会有结果。（但也有用户反馈超过 7 个工作日仍未审核。）
审核过程中可以撤回，除非有严重Bug，不建议撤回。因为重新提交，可能需要重新“排队”。
周末、节假日，微信审核团队会安排少量同事值班。（运气好的话，周末提审，当天也能通过。）
每个小程序，每年有 3 次[加急审核](https://developers.weixin.qq.com/community/develop/doc/00002a9e18cdc0a04669375a95b001 "小程序加急审核流程上线")机会，可在 2 小时内完成审核。
所以只要有一个可发布的版本，尽早提交审核。

审核员工作时间：个人主体类型为 9:00-21:00，非个人主体类型为 9:00-24:00

## 注册小程序的主体
个人和企业均可注册小程序。但是很多类目未对个人开放，限制太多。如果小程序的服务类目不在个人允许的范围内，建议想办法使用企业类型主体（如注册个体工商户）。
...
前面几次审核通过，不代表下一次审核能通过，哪怕只是改了一个Bug。因为后续审核可能发现了之前未发现的“违规”问题。
...
服务类目，尽量在创建小程序时一次性选择全面。因为后面再增加服务类目，可能需要审核，审核时间不确定。

不要在页面提供二维码，供用户下载 App，否则可能会被视为“诱导下载”。

红包类小程序必须选择“社交红包”类目，且需要“增值业务电信许可证”。

涉及到 UGC（用户产生内容），最好建立审核机制与关键词屏蔽功能。
...
如果打算做一个伟大的产品，先把产品名字注册商标，保护起来。因为拥有了商标，即使他人使用了你的名字，也可以很轻松地通过向微信平台申诉拿回名字。
...
- 2019-04-16 小程序激励式视频广告全量开放（此前仅小游戏可开通）
...
一个身份（身份证、手机号、微信号）最多只能作为5个小程序的管理员，因此考虑注销不再运营的小程序，释放管理员名额。
